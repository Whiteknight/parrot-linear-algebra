...
PLA is an add-on for Parrot, so you must have a working, modern version of Parrot installed. Compatibility between versions of PLA and Parrot may be complicated, and many more combinations may exist than have ever been tested. For each supported release of Parrot[1] there will usually be one recommended version of PLA to use. There may not be a recommended version of PLA for every development release of Parrot[2].

fn1. Parrot supported releases are X.0, X.3, X.6, and X.9 version numbers. Supported releases come out every three months (January, April, July, October). Supported Parrot releases might not always be packaged and distributed through your favorite binary package channel.

fn2. Parrot development releases come out monthly, except in months when supported releases are produced. If there are no major API changes in Parrot, the version of PLA recommended for the previous supported release may still work well with the current development release of Parrot. If you absolutely need the bleeding edge for some reason, you're probably going to have to build the software yourself.

*_Parrot_ 2.8 &rarr; _PLA_ 1.0
...
cd kakapo
git checkout PLA_version_1
</pre>

Or, if you don't have git, you can download this particular version of Kakapo in "ZIP":http://github.com/Whiteknight/kakapo/zipball/PLA_version_1 or "TAR":http://github.com/Whiteknight/kakapo/tarball/PLA_version_1 archive formats. Once you have the archive, you can unpack it to see the source.
...
If you have internet access and if your version of Parrot was built with zlib support, you can also submit a smoke report to the "Parrot smolder server":http://smolder.parrot.org/app/projects/smoke_reports/2 . Smoke reports are extremely valuable in tracking bugs and for determining which platforms have good support for PLA and which platforms need more work.
