class Pla::Methods::Transpose is Pla::MatrixTestBase {
    ...
    method test_nonsquare_matrix_transpose() {
        ...
        $m{$!context.factory.key(0,1)} := self.factory.fancyvalue(1);
        $m{$!context.factory.key(0,2)} := self.factory.fancyvalue(2);
        $m{$!context.factory.key(0,3)} := self.factory.fancyvalue(3);

        my $n := $!context.factory.matrix();
        $n{$!context.factory.key(0,0)} := self.factory.fancyvalue(0);
        $n{$!context.factory.key(1,0)} := self.factory.fancyvalue(1);
        $n{$!context.factory.key(2,0)} := self.factory.fancyvalue(2);
        $n{$!context.factory.key(3,0)} := self.factory.fancyvalue(3);
